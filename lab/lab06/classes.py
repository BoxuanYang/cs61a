...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            # Place: (self, name, description, characters, things)
            # characters: a dictionary, name: character
            if person not in characters:
                print(person + " is not here.")

            else:
                print(person + " says: " + characters[person].message)
    ...
    def unlock(self, place):
        ...
        place_obj = self.place.get_neighbor(place)
        if place_obj.locked == False:
