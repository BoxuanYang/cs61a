...
def put_piece(board, max_rows, column, player):
    ...
    for row in range(max_rows):
        ...
        if board[max_rows - 1 - row][column] == '-':
            row_index = max_rows - 1 - row
            new_row = replace_elem(board[max_rows - 1 - row], column, player)
            new_board = replace_elem(board, row_index, new_row)
            break

    return row_index, new_board
    ...
    for row in range(max_rows):
        ...
        for column in range(max_cols):
            if column != max_cols - 1:
                string += get_piece(board, row, column) + " "
            else:
                string += get_piece(board, row, column)
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    row_win = check_win_column(board, max_rows, max_cols, num_connect, col, player)
    col_win = check_win_row(board, max_rows, max_cols, num_connect, row, player)
