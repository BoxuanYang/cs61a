...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        # Ant
        if insect.is_ant:
            # if there is no ant in current place
            if self.ant is None:
                self.ant = insect
                insect.place = self

            # there is already an ant
            else:
                # BEGIN Problem 9

                # if the current ant can contain another ant
                if self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)

                # if the passed ant can contain another ant
                elif insect.can_contain(self.ant):
                    insect.contain_ant(self.ant)
                    self.ant = insect

                # if neither can contain other
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while not place.bees and place != hive:
            if i >= self.max_range:
                break
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.isTrueQueen = (QueenAnt.instance == 0)
        QueenAnt.instance += 1
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # if this is a fake Queen
        if not self.isTrueQueen:
            self.reduce_armor(self.armor)
            return

        # Doubles the Ants' damage behind it
        place = self.place.exit
        while place is not None:
            if place.ant is not None:
                if not place.ant.hasDoubled:
                    place.ant.damage *= 2
                    place.ant.hasDoubled = True

                # if it contains other ant -----may have bugs
                if place.ant.container:
                    if place.ant.ant is not None and not place.ant.ant.hasDoubled:
                        place.ant.ant.damage *= 2
                        place.ant.ant.hasDoubled = True

            place = place.exit

        # Normal ScubaThrower behavior
