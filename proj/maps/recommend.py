...
def group_by_centroid(restaurants, centroids):
    ...
    for i in range(len(restaurants)):
        #location -> a sequence of [x,y]
        location = find_closest(restaurant_location(restaurants[i]), centroids)
        l.append([location, restaurants[i]])
...
def find_centroid(cluster):
    ...
    xLoc = [location[0] for location in locations]
    yLoc = [location[1] for location in locations]
    xMean = mean(xLoc)
    yMean = mean(yLoc)
    return [xMean, yMean]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    """
    # A dictionary of which is sth like { 'res_name': rating}
    reviews_by_user = {review_restaurant_name(review): review_rating(review)
                       for review in user_reviews(user).values()}

    # A list of function value for all restaurants
    xs = [feature_fn(r) for r in restaurants]
    # A list of ratings of all restaurants
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # A list of restaurants ---> the training set
...
def search(query, restaurants):
    ...
    l = [restaurant for restaurant in restaurants if query in restaurant_categories(restaurant)]
    return l
    # END Question 10
